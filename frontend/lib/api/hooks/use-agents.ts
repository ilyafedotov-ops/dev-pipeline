  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: queryKeys.agents.project(variables.projectId || 0),
      });
    },
  });
}
...
export function useUpdateAgentAssignments() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({
        queryKey: queryKeys.agents.assignments(variables.projectId),
      });
      queryClient.invalidateQueries({ queryKey: queryKeys.agents.list(variables.projectId) });
    },
  });
...
export function useUpdateAgentPrompt() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      if (projectId) {
        return apiClient.put<AgentPromptTemplate>(
          `/agents/projects/${projectId}/prompts/${promptId}`,
          data
        );
      }

 ...
 */
export function EventFeed({ protocolId, projectId }: EventFeedProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return filteredByType.filter((e) => {
      const blob =
        `${e.event_type} ${e.message} ${e.project_name ?? ""} ${e.protocol_name ?? ""}`.toLowerCase();
    ...
    <Card>
      ...
      <CardContent className="space-y-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="text-muted-foreground text-xs">{filtered.length} shown</div>
        </div>

        {filtered.length === 0 ? (
          <div className="text-muted-foreground py-6 text-sm">No events yet.</div>
        ) : (
...
function getEventIcon(eventType: string, category?: string | null) {
  ...
  if (type.includes("git_pull") || type.includes("pr_") || type.includes("pull_request"))
...
function renderEventDetails(
    ...
    return (
      <div className="space-y-1">
        <div className="flex items-center gap-2">
          ...
          <code className="bg-muted rounded px-1.5 py-0.5 text-xs">{String(m.branch)}</code>
          {typeof m.base_ref === "string" && m.base_ref && (
            <>
              <span className="text-muted-foreground">from</span>
              <code className="bg-muted rounded px-1.5 py-0.5 text-xs">{String(m.base_ref)}</code>
            </>
          )}
        </div>
  ...
  if (type.includes("protocol_") || type.includes("step_")) {
    ...
    if (m.duration_ms)
      {details.push({ label: "Duration", value: formatMetadataValue("duration", m.duration_ms) });}
    if (m.cost_tokens)
      {details.push({ label: "Tokens", value: formatMetadataValue("tokens", m.cost_tokens) });}
    if (m.cost_cents)
      {details.push({ label: "Cost", value: `$${(Number(m.cost_cents) / 100).toFixed(4)}` });}
  ...
  if (type.includes("qa_") || type.includes("quality_")) {
    return (
      <div className="space-y-1">
        {typeof m.gate_name === "string" && m.gate_name && (
          <div className="flex items-center gap-2">
        ...
        {Array.isArray(m.findings) && m.findings.length > 0 && (
          <div className="text-muted-foreground text-xs">{m.findings.length} finding(s)</div>
        )}
        {m.score !== undefined && (
  ...
  const displayKeys = Object.keys(m).filter(
    (k) =>
      importantKeys.some((ik) => k.toLowerCase().includes(ik)) ||
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs md:grid-cols-3">
        {displayKeys.slice(0, 6).map((key) => (
          <div key={key} className="flex gap-1">
            ...
            <span className="truncate font-medium">{formatMetadataValue(key, m[key])}</span>
          </div>
        ))}
      </div>
    );
        ...
        <div className="min-w-0 flex-1 space-y-2">
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs">
            ...
            {eventHasProtocolLink(e) && (
              <Link
                href={`/protocols/${e.protocol_run_id}`}
          ...
          {hasMetadata && (
            <Collapsible open={showRaw} onOpenChange={setShowRaw}>
              <CollapsibleTrigger asChild>
                ...
                >
                  <ChevronDown
                    className={cn("mr-1 h-3 w-3 transition-transform", showRaw && "rotate-180")}
                  />

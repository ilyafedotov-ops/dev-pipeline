...
from tasksgodzilla.logging import EXIT_RUNTIME_ERROR, EXIT_OK, init_cli_logging, json_logging_from_env, get_logger
from tasksgodzilla.spec_tools import audit_specs
from tasksgodzilla.storage import create_database

DEFAULT_API_BASE = os.environ.get("TASKSGODZILLA_API_BASE", "http://localhost:8011")
DEFAULT_TOKEN = os.environ.get("TASKSGODZILLA_API_TOKEN")
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Interactive CLI for the TasksGodzilla orchestrator. Run with no args for interactive mode.",
    )
    parser.add_argument("--api-base", default=None, help=f"API base URL (default: {DEFAULT_API_BASE})")
    parser.add_argument("--token", default=None, help="Bearer token (or TASKSGODZILLA_API_TOKEN)")
    parser.add_argument("--project-token", default=None, help="Project token (or TASKSGODZILLA_PROJECT_TOKEN)")
...
def run_cli(argv: Optional[List[str]] = None, *, transport: Optional[Any] = None) -> int:
    ...
    init_cli_logging(os.environ.get("TASKSGODZILLA_LOG_LEVEL", "INFO"), json_output=json_logging_from_env())
